...
struct mg_connection *mg_socketcan_listen(struct mg_mgr *mgr, const char *url, // rox 
                                     mg_event_handler_t fn, void *fn_data) {
  int port = atoi(url+3);
  struct mg_connection *c = mg_listen(mgr, url, fn, fn_data);
  // if (c != NULL) c->pfn = http_cb;
  
  if (c != NULL) c->loc.port = port; // c->pfn_data = url;
...
void mg_hexdump_sk(const char* header, const void *buf, size_t len) {
  if(strlen(header)>0)
	  logs((char *) header, strlen(header));
  ...
  for (i = 0; i < len; i++) {
    if ((i % 16) == 0) {
      // Print buffered ascii chars
      // if (i > 0) logs("  ", 2), logs((char *) ascii, 16), logc('\n'), alen = 0;
      if (i > 0) logs("  ", 2), logs((char *) ascii, 16), alen = 0;
      // Print hex address, then \t
      logc(nibble((i >> 12) & 15)), logc(nibble((i >> 8) & 15)),
          logc(nibble((i >> 4) & 15)), logc('0'), logs("   ", 3);
    }
...
static bool mg_atone(struct mg_str str, struct mg_addr *addr) {
  if(strncmp(str.ptr, "can", 3) == 0) // rox mg_atone
  {    
      // addr->port =  atoi(str.ptr+3);
      addr->mg_protocol = MG_PROTOCAL_SOCKETCAN;
      return true; 
  }
...
struct mg_connection *mg_listen(struct mg_mgr *mgr, const char *url,
  ...
  if ((c = mg_alloc_conn(mgr)) == NULL) {
    ...
    c->is_udp = strncmp(url, "udp:", 4) == 0;
    c->is_listening = 1; 
    if(strncmp(url, "can", 3) == 0){ 
      c->is_readable = c->is_writable = 1; // rox mg_listen
      // c->is_closing = 0;
    }
...
union usa {
  ...
  struct sockaddr_can sa_can; // rox 
#if MG_ENABLE_IPV6
...
static socklen_t tousa(struct mg_addr *a, union usa *usa) {
  ...
  // if(a->mg_protocol == MG_PROTOCAL_SOCKETCAN){
        ...
        // usa->sa_can.can_family = AF_CAN;
        // usa->sa_can.can_ifindex = ifr.ifr_ifindex;

      
        // /* memset(usa, 0, sizeof(*usa));
        // usa->sin.sin_family = AF_CAN;
...
bool mg_open_listener(struct mg_connection *c, const char *url) {
  ...
  else if(c->loc.mg_protocol == MG_PROTOCAL_SOCKETCAN){ // rox mg_open_listener
        int rc, on = 1, af = PF_CAN;
        int type = SOCK_RAW;
        int proto = CAN_RAW;
        c->is_listening = 1;
        c->is_readable = 1;
...
#endif
        }else if ((rc = setsockopt(fd, SOL_CAN_RAW, CAN_RAW_FD_FRAMES, (char *) &on, sizeof(on))) != 0) {
            MG_ERROR(("setsockopt(CAN_RAW_FD_FRAMES): %d", MG_SOCK_ERR(rc)));
        }
        else{ 
            strcpy(ifr.ifr_name, url );
...
static void read_conn(struct mg_connection *c) {
  if (ioalloc(c, &c->recv)) {
    ...
    MG_VERBOSE(("%lu %p snd %ld/%ld rcv %ld/%ld n=%ld err=%d", c->id, c->fd,
...
void mg_mgr_poll(struct mg_mgr *mgr, int ms) {
  ...
  for (c = mgr->conns; c != NULL; c = tmp) {
    ...
    } else if (c->is_listening && c->is_udp == 0 && c->loc.mg_protocol != MG_PROTOCAL_SOCKETCAN) { // rox mg_mgr_poll
...
unsigned short mg_url_port(const char *url) {
  ...
  if (strncmp(url, "can", 3) == 0) port = atoi(url+3); // rox  socketcan
