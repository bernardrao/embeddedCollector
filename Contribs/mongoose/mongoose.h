...
#include <linux/can.h>    // rox 
#include <linux/can/raw.h>  // rox
...
void mg_hexdump_sk(const char* header,const void *buf, size_t len); // rox 
...
enum {  // mg_protocol
    MG_PROTOCAL_TCPIPv4,        // ipv4 
    MG_PROTOCAL_TCPIPv6,        // ipv6 
    MG_PROTOCAL_UDP,            // UDP 
    MG_PROTOCAL_SOCKETCAN,      // socket can bus
    MG_PROTOCAL_TTY             // serial port 
};
...
struct mg_connection {
  ...
  // unsigned is_socketcan : 1;   // SOCKET CAN connection
...
struct mg_connection *mg_http_listen(struct mg_mgr *, const char *url,
                                     mg_event_handler_t fn, void *fn_data);
// rox                                      
struct mg_connection *mg_socketcan_listen(struct mg_mgr *, const char *url,
                                     mg_event_handler_t fn, void *fn_data);
